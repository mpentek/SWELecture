...
class TimeIntegrationGeneralizedAlphaScheme(TimeIntegrationBaseScheme):
    ...
    def __init__(self, scheme_settings):
        ...
        super(TimeIntegrationGeneralizedAlphaScheme,
              self).__init__(scheme_settings["settings"])
    ...
    def Initialize(self, model):
        ...
        self.buffer[3, 0, :] = np.dot(model.m, self.buffer[2, 0, :]) + np.dot(
            model.b, self.buffer[1, 0, :]) + np.dot(model.k, self.buffer[0, 0, :])
        self.buffer[3, 1, :] = np.dot(model.m, self.buffer[2, 1, :]) + np.dot(
            model.b, self.buffer[1, 1, :]) + np.dot(model.k, self.buffer[0, 1, :])
    ...
    def _AssembleRHS(self, model):
        ...
        f = (1.0 - self.alphaF) * self.force + \
            self.alphaF * self.buffer[3, 1, :]
        RHS = np.dot(model.m, (self.a1m * self.buffer[0, 1, :] + self.a2m *
                               self.buffer[1, 1, :] + self.a3m * self.buffer[2, 1, :]))
        RHS += np.dot(model.b, (self.a1b * self.buffer[0, 1, :] + self.a2b *
                                self.buffer[1, 1, :] + self.a3b * self.buffer[2, 1, :]))
    ...
    def UpdateDerivedValues(self):
        """
        """
        self.buffer[1, 0, :] = self.a1v * (self.buffer[0, 0, :] - self.buffer[0, 1, :]) + \
            self.a2v * self.buffer[1, 1, :] + self.a3v * self.buffer[2, 1, :]
        self.buffer[2, 0, :] = self.a1a * (self.buffer[0, 0, :] - self.buffer[0, 1, :]) + \
            self.a2a * self.buffer[1, 1, :] + self.a3a * self.buffer[2, 1, :]
