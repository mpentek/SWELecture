...
from source import mdof_solver
...
class DynamicAnalysis(AnalysisType):
    ...
    def animate_time_history(self):
        ...
        visualize_result_utilities.animate_result(plot_title,
                                                  ...
                                                  force,
                                                  scaling)
class DynamicAnalysisModalSDOF(AnalysisType):
    """
    Dervied class for the dynamic analysis of a given structure model        

    """

    def __init__(self, structure_solver, force, time_settings, eigen_vector, eigen_values, name="DynamicAnalysis"):
        ...
        self.structure_solver.model.b = damping_normalised
    ...
    def solve(self):
        ...
        sdof_settings = {
        "solver_type" : "sdof",
        "input_file"  : "ProjectParametersSDoFModal"
        }

        sodf_solver_modal = mdof_solver.MDoFSolver(sdof_settings, 1)
        # dof loop 
        for j in range(len(self.eigen_values)): 

            sodf_solver_modal.model.m = np.array([[self.structure_solver.model.m[j,j]]])
            sodf_solver_modal.model.k = np.array([[self.structure_solver.model.k[j,j]]])
            sodf_solver_modal.model.b = np.array([[self.structure_solver.model.b[j,j]]])
            
            # time loop
            sodf_solver_modal.Initialize()
            for i in range(1, len(self.array_time)):
                current_time = self.array_time[i]
                sodf_solver_modal.ApplyExternalLoad(np.array(self.force[j, i]))
                sodf_solver_modal.SolveSolutionStep()

                # appending results to the list
                self.displacement[j, i] = sodf_solver_modal.GetSolutionStepValue(
                    "DISPLACEMENT")
                self.velocity[j, i] = sodf_solver_modal.GetSolutionStepValue(
                    "VELOCITY")
                self.acceleration[j, i] = sodf_solver_modal.GetSolutionStepValue(
                    "ACCELERATION")
                # update results
                sodf_solver_modal.AdvanceInTime(current_time)
        
        # transforming back from modal coordinates 
        displacement_normal = np.matmul(self.eigen_vector,self.displacement)
        self.displacement = displacement_normal
        velocity_normal = np.matmul(self.eigen_vector,self.velocity)
        self.velocity = velocity_normal
        acceleration_normal = np.matmul(self.eigen_vector,self.acceleration)
        self.acceleration = acceleration_normal
...
class DynamicAnalysisModal(AnalysisType):
    """
    Dervied class for the dynamic analysis of a given structure model        
