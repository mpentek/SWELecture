...
# -------------------------------------------------------------
'''
structural_solver = StructuralMechanicsAnalysis(
    structural_model, parameters_structure)
...
mapper = fsi_utilities.CreateMapper(
    fluid_model_part, structural_model_part, parameters_fsi["coupling_settings"]["mapper_settings"])
...
file_writer = other_utilities.FileWriter(
    "generic_fsi.dat", ["Time", "Disp_X", "Disp_Y", "Disp_Z", "Coupling_Iterations"])
# the point where displacements are traced over time
point_of_interest = KratosMultiphysics.Point(15.0, 190.0, 0.0)
node_id = fsi_utilities.GetNodeid(structural_model_part, point_of_interest)
tip_node = structural_model_part.GetNode(node_id)
...
while(time <= end_time):
    ...
    old_displacements = fsi_utilities.GetDisplacements(
        mapper.origin_interface.Nodes, DIMENSION)
    ...
    for k in range(convergence_accelerator.max_iter):
        ...
        # Mesh and Fluid are solved.
        # Solve Mesh and Fluid
        ...
        # KratosStructuralMechanics.POINT_LOAD
        fsi_utilities.NeumannToStructure(mapper, True)
        ...
        if convergence_accelerator.max_iter > 1:
            displacements = fsi_utilities.GetDisplacements(
                mapper.origin_interface.Nodes, DIMENSION)
            ...
            if (fsi_utilities.Norm(residual) <= convergence_accelerator.res_rel_tol):
                fsi_utilities.SetDisplacements(
                    displacements, mapper.origin_interface.Nodes, DIMENSION)
                ...
                fsi_utilities.SetDisplacements(
                    relaxed_displacements,  mapper.origin_interface.Nodes, DIMENSION)
