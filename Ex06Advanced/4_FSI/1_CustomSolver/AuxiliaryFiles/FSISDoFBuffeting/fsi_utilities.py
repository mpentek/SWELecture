...
def GetNodeid(structural_model_part, node_coord):
    '''
    Gets the node id of the given coordinate 
    '''
    for node in structural_model_part.Nodes:
        if ((node.X0 - node_coord[0])**2 + (node.Y0 - node_coord[1])**2)**0.5 < 1e-4:
            node_id = node.Id
    return node_id
...
def NeumannToStructure(mapper, flag):
    ...
    mapper.InverseMap(mapper.inverse_map["origin_variable"].GetString(), mapper.inverse_map["destination_variable"].GetString(), flag)

def DisplacementToMesh(mapper):
    #mapper.Map(KratosMultiphysics.DISPLACEMENT, KratosMultiphysics.MESH_DISPLACEMENT)
    mapper.Map(mapper.map["origin_variable"].GetString(), mapper.map["destination_variable"].GetString())
...
class CustomMapper(object):
    ...
    def _GetCouplingMatrix(self):
        ...
        for destination_node in self.destination_interface.Nodes:
            ...
            print("NONMATCHING " + "(fluid) destination node", destination_node.Id, "paired with (structure) origin node", id_1, "& ", id_2)

            print("======================================================================")
            print("||||||||||||||||||||||||INTERFACE PAIRING DONE||||||||||||||||||||||||")
            print("======================================================================")
