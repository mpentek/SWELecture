...
class DynamicAnalysis(AnalysisType):
    ...
    def __init__(self, structure_solver, force, time_settings, name="DynamicAnalysis"):
        ...
        RecursivelyValidateAndAssignDefaults(default_settings, time_settings)   # check how to do this 
        ...
        self.array_time = np.arange(self.start_time, self.end_time + self.dt, self.dt)
        rows = len(self.force)
    ...
    def solve(self):
        ...
        for i in range(1,len(self.array_time)): 
            ...
            self.displacement[:,i] = self.structure_solver.GetSolutionStepValue("DISPLACEMENT")
            self.velocity[:,i] = self.structure_solver.GetSolutionStepValue("VELOCITY")
            self.acceleration[:,i] = self.structure_solver.GetSolutionStepValue("ACCELERATION")
            # update results
    ...
    def plot_selected_time_step(self, selected_time_step):
        ...
        plot_title = "Dyanimc Analyis: Deformation at t = " + str(self.array_time[idx]) + " [s]"       
    ...
    def animate_time_history(self):
        ...
        self.structure_solver.model.nodal_coordinates["x"] = [[] for i in range(len(self.array_time))]
        self.structure_solver.model.nodal_coordinates["y"] = [[] for i in range(len(self.array_time))]
        for i in range(len(self.array_time)):
            ...
            self.structure_solver.model.nodal_coordinates["y"][i] = self.structure_solver.model.nodal_coordinates["y0"]
