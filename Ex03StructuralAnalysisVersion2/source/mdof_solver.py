...
from source.co_simulation_base_solver import CoSimulationBaseSolver
from source.custom_files import RecursivelyValidateAndAssignDefaults

# Importing models and schemes
from source.mdof.solver_models import *
from source.mdof.schemes import *
...
class MDoFSolver(CoSimulationBaseSolver):
    def __init__(self, cosim_solver_settings, level):
        ...
        scheme_module = __import__(
            "time_integration_" + scheme_type + "_scheme")
        ...
        if "settings" in parameters["time_integration_scheme_parameters"]:
            parameters["time_integration_scheme_parameters"]["settings"].update(
                {"nr_of_dofs": len(self.model.u0)})
        else:
            parameters["time_integration_scheme_parameters"].update(
                {"settings": {"nr_of_dofs": len(self.model.u0)}})
        ...
        RecursivelyValidateAndAssignDefaults(
            default_solver_settings, parameters["solver_parameters"])
        # output settings
        RecursivelyValidateAndAssignDefaults(
            default_output_settings, parameters["output_parameters"])
    ...
    def OutputSolutionStep(self):
        with open(self.output_file_name, "a") as results_sdof:
            ...
            results_sdof.write(str(self.time) + "\t" + " ".join(str(value)
                                                                for value in self.buffer[0, 0, :]) + "\n")
    ...
    def ApplyExternalLoad(self, force):
        self.scheme.ApplyExternalLoad(force)
